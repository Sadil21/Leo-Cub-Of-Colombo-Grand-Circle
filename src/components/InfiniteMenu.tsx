  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        const offsetX = x + (cellSize - drawWidth) * 0.5;
        const offsetY = y + (cellSize - drawHeight) * 0.5;
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  const handleButtonClick = () => {
    ...
    if (
      ...
      activeItem.link.startsWith('data:') ||
      activeItem.link.startsWith('blob:')
    ) {
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        ...
        `}
          ...
          absolute
          max-w-[22ch]
          text-[clamp(0.92rem,1.4vw,1.5rem)]
          leading-[1.1]
          bottom-22
          left-4
          right-4
          ...
          ${
            isMoving
              ? 'opacity-0 pointer-events-none duration-[100ms] translate-x-0 translate-y-0 md:translate-x-[-60%] md:-translate-y-1/2'
              : 'opacity-100 pointer-events-auto duration-[500ms] translate-x-0 translate-y-0 md:translate-x-[-90%] md:-translate-y-1/2'
          }
        `}
          >
          ...
          grid
          place-items-center
          bg-[linear-gradient(180deg,rgba(124,92,255,0.95),rgba(69,43,236,0.92))]
          border
          border-white/20
          rounded-full
          cursor-pointer
          shadow-[0_20px_40px_rgba(64,32,196,0.45)]
          backdrop-blur-xl
          transition-all
          ease-[cubic-bezier(0.25,0.1,0.25,1.0)]
          ${
        ...
        `}
          >
            <p className="select-none relative text-white top-[1px] text-[25px]">&#x2197;</p>
          </div>
        </>
      )}
    </div>
